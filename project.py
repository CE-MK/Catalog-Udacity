...
APPLICATION_NAME = "Restaurant Menu Catalog
...
def gconnect():
    ...
    # In case there was an error in the access token.
    if result.get('error') is not None:
        response = make_response(json.dumps(result.get('error')), 500)
        response.headers['Content-Type'] = 'application/json'
        return response

    # Check and verify that the access token is used for the intended user.
    ...
    # Check and Verify that the access token is valid for this app.
